...
vector<CPM::CELL_ID> InitRectangle::setRegularAlternative()
{
	vector<CPM::CELL_ID> cells;
	...
	if (l->getDimensions() == 2)	// wenn "2D", dann...
	{
		...
		for(int i = 0; i < number_of_cells; i++)
		{
			int line_position  =   (double(i) + 0.5) * cell_distance;
			VDOUBLE newPos;						//neue knoten an position anlegen
			newPos.x = origin.x + line_position % size.x;
			newPos.y = origin.y + (double (line_position /  size.x) + 0.5 ) * line_distance;
			newPos.z = 0;
